    ...
    <div className="space-y-3">
      ...
      <Card className="bg-gradient-to-br from-emerald-500 to-emerald-700 text-white border-0">
        <CardContent className="p-4 space-y-3">
          <div className="text-center">
            ...
            <p className="text-3xl font-bold">{availableBalance.toLocaleString()} F</p>
          </div>
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="bg-white/10 rounded-lg p-2">
              <p className="text-sm font-bold">{todayEarnings.toLocaleString()} F</p>
              <p className="text-[9px] text-emerald-100">Aujourd'hui</p>
            </div>
            <div className="bg-white/10 rounded-lg p-2">
              <p className="text-sm font-bold">{weekEarnings.toLocaleString()} F</p>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Card className="p-3 text-center">
          ...
          <p className="text-sm font-bold">{totalEarnings.toLocaleString()} F</p>
          <p className="text-[9px] text-muted-foreground">Total gagné</p>
        </Card>
        <Card className="p-3 text-center">
          <ArrowDownToLine className="w-4 h-4 mx-auto text-blue-500 mb-1" />
          <p className="text-sm font-bold">{totalWithdrawn.toLocaleString()} F</p>
      ...
      <Card>
        ...
        <CardContent className="p-3 pt-1 space-y-2.5">
          <div>
            ...
            <Input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder={`Max: ${availableBalance.toLocaleString()}`} />
          </div>
          <div>
          ...
          <CardContent className="p-3 pt-1 space-y-2">
            {withdrawals.map((w: any) => (
              <div key={w.id} className="flex items-center justify-between p-2 rounded-lg bg-muted/50">
                <div>
                  <p className="text-sm font-medium">{w.amount.toLocaleString()} F</p>
