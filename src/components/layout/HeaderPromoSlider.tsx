...
const HeaderPromoSlider = () => {
  ...
  return (
    <div className="bg-background pt-2 sm:pt-3 md:pt-0 pb-3 sm:pb-5 space-y-3 sm:space-y-4">
      {/* === MOBILE — full background image slider === */}
      <div className="sm:hidden px-3">
        <div className="relative overflow-hidden rounded-xl bg-foreground shadow-xl min-h-[230px]">
            ...
            {slides.map((slide, i) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-r from-foreground/88 via-foreground/62 to-foreground/18" />
                  <div className="absolute inset-0 bg-gradient-to-t from-foreground/72 via-transparent to-transparent" />
                  <div className="relative z-10 h-full flex flex-col justify-center px-4 py-5 pr-16">
                      <span className="inline-flex self-start items-center gap-1 bg-primary-foreground/95 text-primary px-2 py-0.5 rounded-none text-[9px] font-extrabold uppercase tracking-wider mb-2 shadow-sm">
                        <Sparkles className="w-2.5 h-2.5" />
                        {slide.eyebrow}
                      </span>
                      <h3 className="font-heading font-black leading-[1.05] text-[21px] text-primary-foreground drop-shadow-md">
                        {head && <>{head}<br /></>}
                        <span className="text-accent">{tail}</span>
                      </h3>
                      <p className="text-primary-foreground text-[11px] mt-1.5 leading-snug line-clamp-2 max-w-[27ch] drop-shadow">
                        {slide.subtitle}
                      </p>
                      <Link to="/marketplace" className="mt-3 self-start">
                        <span className="inline-flex items-center gap-1.5 bg-primary-foreground text-foreground rounded-full pl-3 pr-1 py-1 text-[11px] font-bold shadow-md hover:shadow-lg transition-shadow">
                          Explorer maintenant
            ...
            {slides.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${
                  i === current ? "w-6 bg-accent" : "w-1.5 bg-primary-foreground/65"
                }`}
              />
            ))}
          </div>
