...
const Header = () => {
  ...
  const { onSearchFocus, onSearchType } = useSearchSuggestionsPrefetch();
            ...
            <div className="flex items-center justify-between h-12 sm:h-14 gap-2">
              <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
                ...
                <SheetContent side="left" className="w-80 p-0">
                  <SheetHeader className="px-4 py-2 border-b border-border bg-primary text-primary-foreground">
                    <SheetTitle className="flex items-center justify-start text-primary-foreground">
                      <img src={nukuLogoWhite} alt="NUKUCONNECT" className="h-10 sm:h-11 w-auto object-contain" />
                    </SheetTitle>
                  </SheetHeader>
              ...
              <Link to="/" className="flex items-center flex-shrink-0">
                ...
                <div className="relative w-full">
                  <Input type="text" placeholder="Rechercher un produit ou une catégorie..." value={searchQuery}
                    ...
                    className="w-full h-10 pl-4 pr-52 rounded-full bg-primary-foreground text-foreground placeholder:text-muted-foreground border-0 text-sm" />
      ...
      <div className="lg:hidden bg-primary px-3 pb-2 pt-1" ref={mobileSearchRef}>
        <div className="relative flex items-center gap-1.5">
          <div className="relative flex-1">
              ...
              onChange={(e) => { setSearchQuery(e.target.value); setShowSearchResults(true); onSearchType(); }}
              onFocus={() => { setShowSearchResults(true); onSearchFocus(); }}
              className="w-full h-9 pl-4 pr-32 rounded-full bg-primary-foreground/90 text-foreground placeholder:text-muted-foreground border-0 text-xs" />
            <div className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center gap-0.5">
              <button type="button" onClick={() => setQrScannerOpen(true)}
      ...
      <nav className="hidden lg:block bg-card border-b border-border z-40">
          ...
          <div className="container mx-auto px-4">
            <div className="flex items-center h-10 gap-1">
              ...
              <div className="relative group/cat flex-shrink-0">
                <Button
                  variant="ghost"
                  className="gap-2 text-foreground hover:bg-muted h-8 px-3 text-xs font-medium"
                ...
                {categoriesOpen && (
                  <div
                    className="absolute top-full left-0 z-50 flex bg-card border border-border rounded-lg shadow-xl mt-0.5"
                    onMouseLeave={() => setCategoriesOpen(false)}
                  >
                    {/* Left: category list */}
                    <div className="w-[240px] border-r border-border py-2 max-h-[70vh] overflow-y-auto">
                      <Link
                        to="/categories"
                        className="flex items-center justify-between px-4 py-2 text-xs font-semibold text-primary hover:bg-muted"
                    ...
                    {/* Right: subcategories panel - shows for hovered category */}
                    <CategoriesMegaPanel
                      categories={marketplaceCategories.filter((c: any) => c.is_active)}
              ...
              <div className="flex items-center gap-0.5 flex-1 min-w-0">
                {navLinks.slice(0, 7).map((link) => (
                  <Link key={link.href + link.label} to={link.href}
                    className={`px-2.5 py-1.5 text-xs font-semibold uppercase tracking-wide transition-all duration-200 rounded-md whitespace-nowrap flex-shrink-0 ${
                      ...
                      <div className="grid grid-cols-2 gap-1">
                        {navLinks.slice(7).map((link) => (
                          <DropdownMenuItem key={link.href + link.label} asChild className="cursor-pointer">
                            <Link to={link.href} className="flex items-center gap-2 px-3 py-2.5 rounded-lg hover:bg-muted text-xs font-semibold uppercase tracking-wide">
