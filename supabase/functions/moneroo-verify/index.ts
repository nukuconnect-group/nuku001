...
async function finalizePayment(admin: any, tx: any, paymentId: string) {
  ...
  if (context === "cart") {
    ...
    if (orderIds.length) {
      await admin.from("orders").update({ status: "confirmed", notes: `Paiement Moneroo confirmé | ${paymentId}` }).in("id", orderIds).eq("status", "pending");
    }
    await admin.from("notifications").insert({ user_id: userId, type: "order", title: "✅ Commande confirmée !", description: "Votre paiement Moneroo est confirmé et votre commande est validée." });
  }
